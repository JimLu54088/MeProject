...
public class ResultController {
	...
	public ResponseEntity<?> getSearchResultList(@RequestParam String userId) {
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("Error while getting search result from DB!!");
		}
	...
	public ResponseEntity<?> deleteSavedsearchResult(@RequestBody Map<String, String> requestDeleteBody) {
		...
		logger.debug(LOG_HEADER + "[deleteSavedCriteria] received userId : " + requestDeleteBody.get("user_id")
				+ " received s_r_id : " + requestDeleteBody.get("s_r_id") + " received ins_dt : "
				+ requestDeleteBody.get("ins_dt"));

		try {
			service.deleteSavedsearchResult(requestDeleteBody.get("user_id"), requestDeleteBody.get("s_r_id"),
					requestDeleteBody.get("ins_dt"));
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("Error while delete Saved search result from DB!!");
		}
